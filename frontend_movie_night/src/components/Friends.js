...
export default function AddFriends() {
  ...
  const filterFriends = users.filter(function (filter_users) {
    return (
      friendList.filter(function (filter_friendList) {
        return filter_friendList.id === filter_users.id;
      }).length === 0
    );
  });

  const filteredUser = filterFriends.filter(
    (u) => u.id !== context.loggedInUser.id
  );
  ...
  async function getAllUsers() {
    ...
    let result = await (await fetch("/rest/users", header)).json();
    console.log(result);
    setUsers(result);
  }
  ...
  async function getAllFriends() {
    ...
    let result = await (await fetch("/rest/friends", header)).json();
  ...
  return (
    <div>
      ...
      <Modal isOpen={modal} toggle={toggle}>
        ...
        <ModalFooter>
          <Button color="primary" onClick={sendFriends}>
            Add
          </Button>
